...
def plot_phase2e_results(results_dir):
    ...
    plot_bar_chart(df, "SLA_Violations", "Cumulative SLA Violations by Policy", "Total Violations", "sla_violations_plot.png", results_dir, 'crimson')
    plot_bar_chart(df, "Safety_Violations", "Safety Violations by Policy", "Total Violations", "safety_violations_plot.png", results_dir, 'purple')
    plot_bar_chart(df, "Avg_Total_Control_Latency_ms", "Total Control Latency (ms)", "Latency (ms)", "latency_plot.png", results_dir, 'goldenrod')

    # Instantaneous vs Temporal Repairs Plot
    if "Instantaneous_Repairs" in df.columns and "Temporal_Repairs" in df.columns:
        plt.figure(figsize=(12, 6))
        width = 0.35
        x = np.arange(len(df))
        
        plt.bar(x - width/2, df["Instantaneous_Repairs"], width, label='Instantaneous Repairs', color='darkblue')
        plt.bar(x + width/2, df["Temporal_Repairs"], width, label='Temporal Repairs', color='darkorange')
        
        plt.xticks(x, df["Policy"], rotation=20, ha='right')
        plt.ylabel('Total Repairs')
        plt.title('Instantaneous vs Temporal Shield Repairs')
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, "instantaneous_vs_temporal_repairs_plot.png"), dpi=300)
        plt.close()
    ...
    if os.path.exists(per_step_path):
        ...
        if not ppo_base.empty and "reward_service_quality" in ppo_base.columns:
            ...
            if "penalty_temporal_sla_risk" in ppo_base.columns:
                components["Temporal Risk Penalty"] = ppo_base["penalty_temporal_sla_risk"].mean()
                
            pd.DataFrame([components]).to_csv(os.path.join(results_dir, "reward_component_summary.csv"), index=False)
            
            plt.figure(figsize=(10, 5))
            colors = ['green', 'blue', 'red', 'orange', 'darkred', 'purple'][:len(components)]
    ...
    print("Successfully generated all Phase 2E empirical plots.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--results_dir", type=str, required=True)
    args = parser.parse_args()
    plot_phase2e_results(args.results_dir)
