...
def analyze_phase2d(results_dir):
    per_step_path = os.path.join(results_dir, "seed_1", "eval_shield", "per_step_logs.csv")
    has_steps = False
    
    # Actually we need all per_step_logs. Let's aggregate them
    all_steps = []
    policies = ["PPO without Shield", "PPO with Runtime Shield", "Random Defender", "Static Threshold Defender", "Shield-only policy", "Untrained stochastic policy"]
    ...
    metrics_to_format = [
        ...
        ("Avg_Collateral", "Collateral Damage"),
        ("SLA_Violations", "SLA Violations"),
        ("Safety_Violations", "Safety Violations"),
        ("Shield_Repairs", "Shield Repairs"),
        ("Avg_Total_Control_Latency_ms", "Total Control Latency (ms)")
    ]
    
    latex_lines = [
        "\\begin{table*}[htbp]",
        "\\centering",
        "\\caption{Phase 2D: Final Empirical Evaluation of Sentinel-RTA (Mean $\\pm$ Std Dev)}",
        "\\label{tab:phase2d_results}",
        "\\resizebox{\\textwidth}{!}{",
...
if __name__ == "__main__":
    ...
    analyze_phase2d(args.results_dir)
